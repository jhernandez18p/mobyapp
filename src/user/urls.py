...
urlpatterns = [
    ...
    path('password_reset_complete/done/', auth_views.password_reset_complete, name='password_reset_complete'),
    ...
    re_path(r'^password_reset_confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
        auth_views.password_reset_confirm, {'post_reset_redirect':'auth:password_reset_complete'},name='password_reset_confirm'),
